...
class _ChatPageState extends State<ChatPage> with WidgetsBindingObserver {
  ...
  /// 已有流式正文时不显示「思考中」占位，避免挡住逐字输出
  bool get _showThinkingBubble {
    if (!_isGenerating) return false;
    if (_streamingBlockId != null) {
      if (_streamDirty || _pendingStreamContent.isNotEmpty) return false;
      if (_messages.any((m) => m.id == _streamingBlockId)) return false;
    }
    return true;
  }
  ...
  void _handleLumeChatEvent(Map<String, dynamic>? payload) {
    ...
    if (state == 'start' || state == 'begin') {
      if (runId.isNotEmpty) _streamingBlockId = runId;
      setState(() {
        _queuePosition = 0;
        _queueTotal = 0;
        _isGenerating = true;
    ...
    if (state == 'block' || state == 'delta') {
      ...
      final blockId = runId.isNotEmpty
          ? runId
          : (_streamingBlockId ?? 'lume-stream-${_currentSessionKey ?? 'default'}');
      if (text.isNotEmpty) {
    ...
    if (state == 'final') {
      ...
      if (finalText.trim().isEmpty) {
        // proxy 误发的空 final，流式进行中则忽略
        if (_streamingBlockId != null || _streamDirty || _pendingStreamContent.isNotEmpty) {
          debugPrint('⏭️ 忽略空 final（流式进行中）');
          return;
        }
        _generatingSafetyTimer?.cancel();
        setState(() {
      ...
      final msgId = runId.isNotEmpty
          ? runId
          : (_streamingBlockId ?? DateTime.now().millisecondsSinceEpoch.toString());
      _onStreamEnd();
      _generatingSafetyTimer?.cancel();
      setState(() {
        _isGenerating = false;
                      ...
                      Expanded(
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, i) {
                            try {
                              if (_showThinkingBubble && i == _messages.length) {
