...
class _ServersPageState extends State<ServersPage> {
  ...
  Future<void> _checkServer(String serverId) async {
    try {
      ...
      _loadServers();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('检查完成')));
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _deleteServer(String serverId) async {
    ...
    try {
      ...
      _loadServers();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('设备已删除')));
      }
    } catch (e) {
      if (mounted) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Text(isEdit ? '编辑设备' : '添加设备', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              Spacer(),
            ...
            SizedBox(height: 4),
            Text('在 OpenClaw 配置文件的 control.token 中找到', style: TextStyle(fontSize: 11, color: Colors.grey)),
            SizedBox(height: 12),
            _buildField('Session ID', sessionCtrl, 'ControlUI URL 中的路径部分'),
            SizedBox(height: 4),
            Text('例如 http://ip:18789/8eb2a992/ 中的 8eb2a992', style: TextStyle(fontSize: 11, color: Colors.grey)),
            SizedBox(height: 12),
              ...
              child: ElevatedButton(
                onPressed: () async {
                  if (ipCtrl.text.trim().isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('IP 地址必填')));
                    return;
                  }
                  final api = ApiService();
                  final uid = _getUserId();
  ...
  Widget _buildField(String label, TextEditingController ctrl, String hint, {TextInputType? keyboard}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF555555))),
        SizedBox(height: 4),
        TextField(
          controller: ctrl,
          ...
          decoration: InputDecoration(
            hintText: hint,
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Color(0xFFDDDDDD))),
            enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Color(0xFFDDDDDD))),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('🖥️ 设备管理'),
        actions: [
          IconButton(icon: Icon(Icons.refresh), onPressed: _loadServers),
        ],
      ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 12),
          Text('还没有设备', style: TextStyle(fontSize: 16, color: Color(0xFF888888))),
          SizedBox(height: 4),
          Text('点击下方按钮添加你的 OpenClaw 服务器', style: TextStyle(fontSize: 13, color: Color(0xFFAAAAAA))),
          SizedBox(height: 24),
          ElevatedButton.icon(
            onPressed: () => _showAddModal(),
            icon: Icon(Icons.add),
            label: Text('添加设备'),
            style: ElevatedButton.styleFrom(backgroundColor: Constants.primaryColor, foregroundColor: Colors.white, padding: EdgeInsets.symmetric(horizontal: 24, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
          ),
        ],
      ),
    );
  ...
  Widget _buildServerCard(Map<String, dynamic> s) {
    ...
    final statusMap = {'running': ('在线', Colors.green), 'offline': ('离线', Colors.grey), 'pending': ('检查中', Colors.orange), 'unhealthy': ('异常', Color(0xFFFB923C))};
          ...
          children: [
            ...
            Row(children: [
              Container(
                width: 42, height: 42,
                decoration: BoxDecoration(
                  color: isActive ? Constants.primaryColor.withOpacity(0.1) : Color(0xFFF0EEFF),
                  borderRadius: BorderRadius.circular(10),
                ),
                child: Center(child: Text(isActive ? '🟢' : '🖥️', style: TextStyle(fontSize: 20))),
              ),
              SizedBox(width: 12),
              Expanded(child: Column(
                ...
                children: [
                  Row(children: [
                    Text(s['name'] ?? '未命名', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                    if (isActive) ...[
                      SizedBox(width: 8),
                      Container(
                        padding: EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                        decoration: BoxDecoration(color: Constants.primaryColor, borderRadius: BorderRadius.circular(4)),
                        child: Text('活跃', style: TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.w600)),
                      ),
                    ],
                  ]),
                  SizedBox(height: 2),
                  Text('${s['ip']}:$port', style: TextStyle(fontSize: 12, color: Color(0xFF888888), fontFamily: 'monospace')),
                  if (s['description'] != null && s['description'].toString().isNotEmpty)
                    Text(s['description'], style: TextStyle(fontSize: 11, color: Color(0xFFAAAAAA))),
                ],
              )),
              // Status dot
              Column(children: [
            ...
            // Actions
            SizedBox(height: 12),
            Container(decoration: BoxDecoration(border: Border(top: BorderSide(color: Color(0xFFF0F0F0))))),
            SizedBox(height: 10),
            Wrap(
              spacing: 8,
              runSpacing: 8,
            ...
            if (s['lastCheck'] != null)
              Padding(
                ...
                child: Text('最后检查: ${_fmtDate(s['lastCheck'].toString())}', style: TextStyle(fontSize: 10, color: Color(0xFFBBBBBB))),
              ),
          ],
        ),
      ),
