...
class _ToolsPageState extends State<ToolsPage> {
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildToolCard(
            ...
            textColor: textColor,
            subColor: subColor,
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => LumeClawPage())),
          ),
          SizedBox(height: 12),
          ...
          _buildToolCard(
            ...
            textColor: textColor,
            subColor: subColor,
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => CronPage())),
          ),
          SizedBox(height: 12),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Text('飞书集成配置', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              Spacer(),
              IconButton(onPressed: () => Navigator.pop(ctx), icon: Icon(Icons.close)),
            ]),
            SizedBox(height: 16),
            Text('在 OpenClaw 配置中设置飞书机器人后，即可通过飞书接收 AI 消息通知。', style: TextStyle(fontSize: 14, color: Colors.grey)),
            SizedBox(height: 16),
  ...
  Widget _buildConfigField(String label, String value) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        ...
        decoration: BoxDecoration(color: Color(0xFFF5F5F5), borderRadius: BorderRadius.circular(8), border: Border.all(color: Color(0xFFE0E0E0))),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Text('📊 使用量统计', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              Spacer(),
              IconButton(onPressed: () => Navigator.pop(ctx), icon: Icon(Icons.close)),
            ]),
            SizedBox(height: 20),
            if (_usageStats != null) ...[
              ...
              Center(child: Text('暂无统计数据', style: TextStyle(color: Colors.grey))),
            ],
            SizedBox(height: 20),
          ],
        ),
  ...
  void _showWecomConfig() {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('企业微信配置开发中...'), backgroundColor: Color(0xFF07C160)),
    );
  }
}
