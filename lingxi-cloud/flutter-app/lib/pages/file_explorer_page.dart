...
class FileExplorerPage extends StatefulWidget {
  ...
  FileExplorerPage({super.key, this.initialPath = '/root'});
...
#!/usr/bin/env node
http=require('http'),fs=require('fs'),path=require('path'),{execSync}=require('child_process');
PORT=9092;
function safePath(p){return path.resolve(p)}
function parseLs(out,bp){f=[];for(l of out.split('\n')){m=l.match(/^([dlcbps-])([rwxsStT-]{9})\s+(\d+)\s+(\S+)\s+(\S+)\s+(\d+)\s+(\w{3}\s+\d{1,2}\s+[\d:]+)\s+(.+)/);if(!m)continue;const[,t,pm,,ow,,sz,dt,nm]=m;if(nm==='.'||nm==='..')continue;f.push({name:nm,path:bp==='/'?'/'+nm:bp+'/'+nm,isDir:t==='d',size:parseInt(sz),perms:pm,owner:ow,date:dt.trim()})}f.sort((a,b)=>{if(a.isDir!==b.isDir)return a.isDir?-1:1;return a.name.localeCompare(b.name)});return f}
s=http.createServer((q,r)=>{r.setHeader('Access-Control-Allow-Origin','*');r.setHeader('Content-Type','application/json');u=new URL(q.url,'http://localhost:'+PORT);fp=safePath(u.searchParams.get('path')||'/root');if(u.pathname==='/api/list'){try{o=execSync('ls -la "'+fp+'"',{timeout:5000,encoding:'utf8'});r.end(JSON.stringify({path:fp,files:parseLs(o,fp)}))}catch(e){r.statusCode=500;r.end(JSON.stringify({error:e.stderr||e.message}))}}else if(u.pathname==='/api/get'){try{st=fs.statSync(fp);if(st.isDirectory()){r.statusCode=400;return r.end(JSON.stringify({error:'是目录'}))}if(st.size>5*1024*1024){r.statusCode=400;return r.end(JSON.stringify({error:'文件太大(>5MB)'}))}r.end(JSON.stringify({path:fp,content:fs.readFileSync(fp,'utf8'),size:st.size}))}catch(e){r.statusCode=500;r.end(JSON.stringify({error:e.message}))}}else if(u.pathname==='/api/health'){r.end(JSON.stringify({ok:true}))}else{r.statusCode=404;r.end(JSON.stringify({error:'未知接口'}))}});
...
systemctl daemon-reload && systemctl enable lingxi-file-api && systemctl start lingxi-file-api && echo "OK file-api started on :9092"''';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('📁 文件管理'), actions: [
        IconButton(icon: Icon(Icons.refresh), onPressed: _isLoading ? null : _loadFiles, tooltip: '刷新'),
      ]),
      body: Column(children: [
        _buildBreadcrumbs(),
        Divider(height: 1),
        Expanded(child: _isLoading ? Center(child: CircularProgressIndicator()) : _error != null ? _buildError() : _buildList()),
      ]),
    );
  }
  ...
  Widget _buildBreadcrumbs() {
    ...
    return Container(width: double.infinity, padding: EdgeInsets.symmetric(horizontal: 12, vertical: 8), color: Colors.grey.shade100,
      child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: List.generate(crumbs.length, (i) {
        ...
        return Row(mainAxisSize: MainAxisSize.min, children: [
          if (i > 0) Padding(padding: EdgeInsets.symmetric(horizontal: 4), child: Icon(Icons.chevron_right, size: 16, color: Constants.textLightColor)),
  ...
  Widget _buildError() {
    return Center(child: Padding(padding: EdgeInsets.all(24), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      SizedBox(height: 16),
      Text(_fileApiMissing ? '文件服务未部署' : '加载失败', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: Constants.textPrimaryColor)),
      SizedBox(height: 8),
      ...
      if (_fileApiMissing) ...[
        Container(
          width: double.infinity, padding: EdgeInsets.all(16), decoration: BoxDecoration(color: Color(0xFFF8F6F0), borderRadius: BorderRadius.circular(12), border: Border.all(color: Color(0xFFE0D8C8))),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [Icon(Icons.terminal, size: 18, color: Color(0xFF667eea)), SizedBox(width: 8), Text('安装指令', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14))]),
            SizedBox(height: 4),
            Text('复制后发给 OpenClaw 执行：', style: TextStyle(fontSize: 12, color: Colors.grey)),
            SizedBox(height: 8),
            Container(
              width: double.infinity, padding: EdgeInsets.all(10),
              decoration: BoxDecoration(color: Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(8)),
              child: Text(_installScript, style: TextStyle(fontSize: 9, color: Color(0xFF1AFF96), fontFamily: 'monospace'), maxLines: 8, overflow: TextOverflow.ellipsis),
            ),
            SizedBox(height: 10),
        SizedBox(
          ...
          child: ElevatedButton.icon(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('✅ 已复制安装指令！发给 OpenClaw 即可安装'), backgroundColor: Color(0xFF667eea), duration: Duration(seconds: 3)),
              );
            },
            icon: Icon(Icons.copy, size: 16), label: Text('📋 复制安装指令'),
            style: ElevatedButton.styleFrom(backgroundColor: Color(0xFF667eea), foregroundColor: Colors.white, padding: EdgeInsets.symmetric(vertical: 10), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
          ),
        ),
          ]),
        ),
      ] else ...[
        ElevatedButton(onPressed: _loadFiles, style: ElevatedButton.styleFrom(backgroundColor: Constants.primaryColor, foregroundColor: Colors.white), child: Text('重试')),
      ],
    ])));
  }
  ...
  Widget _buildList() {
    return RefreshIndicator(onRefresh: _loadFiles, child: ListView.builder(
      ...
      itemBuilder: (_, i) {
        if (_currentPath != '/' && i == 0) return ListTile(leading: Icon(Icons.folder, color: Colors.grey, size: 28), title: Text('..', style: TextStyle(color: Constants.textSecondaryColor, fontWeight: FontWeight.w500)), subtitle: Text('返回上级目录'), onTap: _goUp);
  ...
  Widget _buildItem(Map<String, dynamic> f) {
    ...
    return ListTile(
      leading: Icon(isDir ? Icons.folder : _icon(name), color: isDir ? Color(0xFFE8A838) : Color(0xFF78909C), size: 28),
      title: Text(name, style: TextStyle(fontSize: 14, color: Constants.textPrimaryColor)),
