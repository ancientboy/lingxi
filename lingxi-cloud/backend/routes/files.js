...
/**
 * GET /api/files/tts
 * 获取 TTS 音频文件（公开访问，不需要认证）
 * 
 * Query:
 * - path: 文件路径（如 /tmp/openclaw/tts-xxx/voice.mp3）
 */
router.get('/tts', async (req, res) => {
  try {
    const { path: filePath, userId } = req.query;
    
    if (!filePath) {
      return res.status(400).json({ error: 'Path is required' });
    }

    // 安全检查：只允许访问 /tmp/openclaw 下的文件
    if (!filePath.startsWith('/tmp/openclaw/')) {
      return res.status(403).json({ error: 'Access denied' });
    }

    // 先尝试本地查找
    console.log(`🔊 TTS 本地查找：${filePath}`);
    
    if (fs.existsSync(filePath)) {
      console.log(`✅ TTS 文件在本地`);
    ...
    try {
      ...
      if (fs.existsSync(dbPath)) {
        ...
        // 如果指定了 userId，只查询该用户的服务器
        let targetServers = userServers;
        if (userId) {
          targetServers = userServers.filter(s => s.userId === userId && s.status === 'running');
        } else {
          // 没有 userId，遍历所有运行中的服务器
          targetServers = userServers.filter(s => s.status === 'running');
        }
        
        for (const server of targetServers) {
          const ip = server.ip;
          const port = server.fileServerPort || 9876;
          
          try {
            const response = await axios.get(`http://${ip}:${port}/preview`, {
    ...
    console.error('TTS file error:', err.message);
    res.status(500).json({ error: err.message });
  }
});
