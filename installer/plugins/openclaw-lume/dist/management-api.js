...
const WORKFLOW_DIR = path.join(os.homedir(), ".openclaw", "workspace", "skills", "workflow-hub", "workflows");
...
export async function readWorkflow(workflowId) {
...
export async function writeWorkflow(workflowId, content) {
    const safeId = workflowId.replace(/[^a-zA-Z0-9_-]/g, "");
    const filePath = path.join(WORKFLOW_DIR, `${safeId}.json`);
    if (!filePath.startsWith(WORKFLOW_DIR)) {
        throw new Error("Invalid workflow id");
...
export async function fetchHealthStatus(cfg) {
...
export async function listNativeFiles(dirPath) {
...
export async function sendNativeNotify(title, body) {
