...
function loadDeviceId(openClawDir) {
...
function mergeScopes(existing) {
    const base = Array.isArray(existing) ? existing.filter((s) => typeof s === "string") : [];
    const merged = new Set([...base, ...LUME_OPERATOR_SCOPES]);
    return [...merged];
}
/**
 ...
 */
export function ensureLocalDeviceScopes(log) {
    ...
    let paired;
    try {
        paired = JSON.parse(fs.readFileSync(pairedPath, "utf-8"));
    }
    catch (err) {
    ...
    if (tokens && typeof tokens === "object" && !Array.isArray(tokens)) {
        const operatorToken = tokens.operator;
        if (operatorToken) {
