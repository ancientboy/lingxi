...
export async function sendLumeText(ctx) {
    const userId = ctx.to;
    ...
    if (sessionKey) {
        ...
        if (!bridge)
            throw new Error(`lume: no WebSocket bridge for account ${ctx.accountId ?? "default"}`);
        ...
        if (!sent)
            throw new Error(`lume: user ${userId} is not connected`);
    }
    return { channel: "lume", messageId };
}
export async function sendLumeMedia(ctx) {
    const userId = ctx.to;
    const sessionKey = resolveSessionKey(ctx);
    const messageId = getChatRun(userId, sessionKey || undefined)?.runId
        ?? `lume-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
