...
import type { InboundHandler } from "./ws-bridge.js";
...
interface ChannelRuntimeReply {
  finalizeInboundContext: (ctx: Record<string, unknown>) => Record<string, unknown>;
  resolveHumanDelayConfig: (cfg: unknown, agentId?: string | null) => unknown;
...
interface ChannelRuntimeRouting {
...
interface ChannelRuntimeSession {
...
interface ChannelRuntime {
  routing: ChannelRuntimeRouting;
  session: ChannelRuntimeSession;
  reply: ChannelRuntimeReply;
}

function isValidSessionKey(key: string | undefined): boolean {
  return !!key && key.startsWith("agent:main:");
}

async function pushChatError(
  userId: string,
  accountId: string,
  text: string,
  sessionKey?: string,
...
export function createInboundHandler(
  ctx: ChannelGatewayContext<LumeAccount>,
  gatewayApi?: GatewayApiConfig,
): InboundHandler {
  const { cfg } = ctx;
  const rt = ctx.channelRuntime as unknown as ChannelRuntime | undefined;
  ...
  return async ({ userId, accountId, message, sessionKey, agentId, model, attachments }) => {
    ...
    try {
      ...
      const effectiveSessionKey =
        isValidSessionKey(sessionKey) && sessionKey!.trim().length > 0
          ? sessionKey!
          : userScopedKey;
      ...
      const sessionStore = (cfg as Record<string, unknown>).session as Record<string, unknown> | undefined;
      const storePath = rt.session.resolveStorePath(
        sessionStore?.store as string | undefined,
        { agentId: route.agentId },
      );
      ...
      try {
        await rt.reply.withReplyDispatcher({
          dispatcher,
          run: () =>
            rt.reply.dispatchReplyFromConfig({
              ctx: finalized,
              cfg,
              dispatcher,
              replyOptions: {
