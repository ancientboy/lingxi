/**
 * ws-bridge 管理类 RPC 分发 + 健康状态推送
 * health 默认不主动推送，客户端调用 health.subscribe 后才启动定时器
 */
...
export function stopHealthMonitor(userId: string) {
...
export function startHealthMonitor(
  userId: string,
  ws: WebSocket,
  gatewayApi: GatewayApiConfig | undefined,
  send: SendFn,
) {
  stopHealthMonitor(userId);
  if (!gatewayApi?.token) return;
...
export async function handleManagementMethod(
  method: string,
  ...
  switch (method) {
    ...
    case "workflow.write": {
      ...
      if (!id || !content) throw new Error("workflow id and content required");
      return writeWorkflow(id, content);
    }
    case "health.subscribe": {
      // 客户端主动订阅 → 启动定时推送（返回当前状态作为 RPC 响应）
      if (!gatewayApi?.token) throw new Error("Gateway not configured");
